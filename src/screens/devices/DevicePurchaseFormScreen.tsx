...
export default function DevicePurchaseFormScreen({ navigation }: Props) {
  ...
  const { control, handleSubmit, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: { customerName: '', customerPhone: '', deviceName: '', deviceModel: '', imei: '', purchasePrice: '', notes: '' },
  });
  ...
  return (
    <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        <Text style={styles.section}>Seller (Customer) Info</Text>
        <Controller control={control} name="customerName" render={({ field: { onChange, value } }) => (
          <TextInput label="Seller Name *" value={value} onChangeText={onChange} mode="outlined" style={styles.input} error={!!errors.customerName} />
        )} />
        ...
        <Controller control={control} name="deviceName" render={({ field: { onChange, value } }) => (
          <TextInput label="Device Brand *" value={value} onChangeText={onChange} mode="outlined" style={styles.input} error={!!errors.deviceName} />
        )} />
        ...
        <Controller control={control} name="deviceModel" render={({ field: { onChange, value } }) => (
          <TextInput label="Device Model *" value={value} onChangeText={onChange} mode="outlined" style={styles.input} error={!!errors.deviceModel} />
        )} />
